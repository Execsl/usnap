/********************************************************
 * This utility file contains all of the routines	*
 * needed to impement bit oriented routines under	*
 * either ANSI or K&R C. It needs to be linked with	*
 * every program used in the entire book.		*
 ********************************************************/
...
#include "bitio.h"
//#include "errhand.h"
inline void fatal_error (char *_fe) {}
...
void OutputBits (BIT_FILE *bit_file, unsigned long code, int count) {
  ...
  while (mask != 0) {
    ...
    if (bit_file -> mask == 0) {
      if (putc (bit_file -> rack, bit_file -> file) != bit_file -> rack)
	fatal_error ("Fatal error in OuptutBit! \n");
      else if ((bit_file -> pacifier_counter++ & PACIFIER_COUNT) == 0)
	putc ('.', stdout);
      bit_file -> rack = 0;
      bit_file -> mask = 0x80;
