...
#define GetCurrentProcessId	_GetCurrentProcessId
#include <windows.h>
...
char *mode;
{
  ...
  if (ptr == NULL) {
	  //
	  static char *__map[] = {
		  XPFILE, MAKEINTRESOURCE(IDR_BISON_SIMPLE), 
		  XPFILE1, MAKEINTRESOURCE(IDR_BISON_HAIRY)
	  }; 
	  int __n_map = sizeof(__map) / (sizeof(char *) * 2); 
	  int i; 
	  for (i = 0; i < __n_map; i++) {
		  if (!strcmp(name, __map[i * 2])) {
			  char temppath[MAX_PATH]; 
			  DWORD dw; 
			  
			  dw = GetTempPath(MAX_PATH, temppath); 
			  ASSERT(dw != 0); 
			  dw = GetTempFileName(temppath, "lar", 0, temppath); 
			  ASSERT(dw != 0); 

			  ptr = fopen(temppath, "w+b"); 
			  if (ptr != NULL) {
				  HRSRC hrs; 
				  HGLOBAL h; 
				  DWORD sz; 
				  ...
				  hrs = FindResource(NULL, __map[i * 2 + 1], "RAWDATA"); 
				  ASSERT(hrs != NULL); 
				  
				  h = LoadResource(NULL, hrs); 
				  ASSERT(h != NULL); 
				  
				  sz = SizeofResource(NULL, hrs); 
				  ASSERT(sz > 0); 
				  
				  p = LockResource(h); 
				  ASSERT(p != NULL); 

				  cb = fwrite(p, 1, (size_t)sz, ptr); 
				  ASSERT(cb == sz); 

				  fclose(ptr); 
				  ptr = fopen(temppath, mode); 
